...
const BusinessPage = async () => {
  ...
  const publications = await getPublicationsMock()
  console.log(publications);
  if (!profile || !publications) return (<div>Loading...</div>)
  const { id, name, description, shortDescription } = profile

  return (
    <div className='pb-6'>
      <div className='relative w-full h-full'>
        <div className='w-full h-48 absolute top-0 left-0'>
          <Image src='/kushkiFondo.jpg' className=' w-full object-cover' alt='Profile background' width={400} height={200} />
        </div>
            ...
            <div className='flex flex-col items-center md:items-start lg:flex-row lg:justify-between relative'>
              <div className='flex flex-col md:flex-row items-center relative '>
                ...
                <div className='pt-12 md:pt-2 md:pl-36'>
                  ...
                  <p className='text-gray-700 text-sm'>{profile?.shortDescription ?? 'Empresa'} </p>
                </div>
              </div>
              <div className='flex gap-4 items-center'>
                <div className='pt-2'>
                  editar profile
                  {/* <EditProfileBusinessForm completeProfile={rest} /> */}
                </div>
              </div>
            </div>
            {
